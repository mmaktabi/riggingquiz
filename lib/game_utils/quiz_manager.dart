...
class GameManager {
  ...
  void startListeningToGameStatus({
    required String gameId,
    required void Function(String status, Map<String, dynamic> gameData) onGameStatusChanged,
  }) {
    _gameStatusSubscription = _gameSessionsRef.child(gameId).onValue.listen((event) {
  ...
  void stopListeningToGameStatus() {
  ...
  // ** Methoden aus WaitingRoomService **

  Future<Map<String, dynamic>?> fetchUserData(String userUid) async {
    return await getUserDataMap(userUid);
  }

  StreamSubscription listenForGameStart({
    required String gameId,
    required void Function(String status, Map<String, dynamic> gameData)
    onGameStatusChanged,
  }) {
    return _gameSessionsRef.child(gameId).onValue.listen((event) {
      if (event.snapshot.exists) {
        final gameData = Map<String, dynamic>.from(event.snapshot.value as Map);
        final status = gameData['status'] as String?;
        if (status != null) {
